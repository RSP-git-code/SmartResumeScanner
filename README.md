...
3. Runtime: Python. Build command:
   ...
   Start command:
   ```
   gunicorn resume_screener.wsgi:application --workers 1 --timeout 120
   ```
   `--workers 1` keeps only one full copy of the app (including `chromadb`, whose import chain is unusually heavy — it pulls in `jsonschema`/`referencing` and provider-specific embedding modules) in memory at a time, and `--timeout 120` gives that import room to finish on a free-tier instance instead of gunicorn killing the worker mid-load (observed as a `WORKER TIMEOUT` / `SIGKILL` in the logs on the default 30s timeout, which self-recovered on the next worker but could fail whatever request was in flight).
...
- **Free-tier requests-per-minute cap.** Separately from the daily cap above, the lite-tier model is also capped at 15 requests/minute. Matching several resumes in one batch (2-4 LLM calls each) can burst past that within a single request. `screener/llm/client.py:with_llm_retry` wraps every chain with automatic backoff (up to 4 attempts, a few seconds to ~20s apart) specifically on `GoogleRateLimitError`/429, so a burst retries itself back under the per-minute limit instead of failing that resume's match outright — `ChatGoogleGenerativeAI`'s own `max_retries` isn't enough here since it doesn't wait long enough for a per-minute quota to actually reset.
...
- **Resume de-duplication is content-based, not filename-based.** `Resume.content_hash` is a SHA-256 of the extracted text; re-uploading the same resume (even under a different filename) is detected and skipped rather than creating a second `Resume` row that would otherwise show up as a confusing duplicate, independently-scored entry in every shortlist.

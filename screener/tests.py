...
class CrossRoleRecommendationTests(TestCase):
    ...
    logic over existing Match rows, no LLM call involved, so this is
    testable without hitting the API. See
    screener/views.py:_build_shortlist_context.
    """
...
class JobStatusTests(TestCase):
    ...
    cross-role "may fit better elsewhere" suggestion, but leaves existing
    Match records and its own shortlist untouched (screener/views.py:
    update_job_status, _build_shortlist_context).
    """

...
class JobDescription(models.Model):
    ...
    # target (see views._build_shortlist_context's better_fit_elsewhere) --
...
class Resume(models.Model):
    ...
    # The job whose "Upload Resumes" form this resume came in through, if
    # any. Resumes are still a shared pool matchable against any JD (see
    # resume_pool and the "Also check resume pool" action on run_matching)
    # -- this only controls what "Run Matching" scores *by default* on a
    # given job's page, so uploading a
    # batch of resumes for Job A doesn't silently get re-matched against
    # every other job you happen to click "Run Matching" on (each of those
    # 4+ LLM calls per resume adds up fast, and was overloading Gemini's
    # free tier on jobs that had nothing to do with those candidates).
    # Matching the wider pool against a job is still available, just as an
    # explicit opt-in action instead of the default.
    uploaded_for = models.ForeignKey(
        'JobDescription', on_delete=models.SET_NULL, null=True, blank=True,
        related_name='originally_uploaded_resumes',
    )

...
def resume_pool(request):
    ...
    JD (see run_matching's pool-scope option), but that's easy to lose
    track of from any single job's page. Read-only: matching itself still
    happens from a job's own page (Upload Resumes / Run Matching).
    """
    resumes = Resume.objects.select_related('uploaded_for').order_by('-created_at').prefetch_related(
...
def retry_resume_extraction(request, resume_id):
    """Retries structured extraction + redaction for one resume that never
    finished it (e.g. a transient Gemini failure at upload time) -- see the
    same-content-hash retry in upload_resumes for the more common path via
    re-upload; this covers retrying directly from the Resume Pool page.
    """
    ...
    if request.method == 'POST':
        try:
            ...
            logger.exception('Retry extraction failed for Resume %s', resume.id)
            messages.warning(request, 'Extraction failed again -- check server logs.')
        else:
            messages.success(request, 'Extraction retried successfully.')
    return redirect('resume-pool')
...
def _build_shortlist_context(jd, request):
    ...
    # Split by upload intent -- see Resume.uploaded_for and views.run_matching
    # for why "Run Matching" defaults to only the first group rather than
    # the whole pool.
    own_unmatched_resumes = [r for r in unmatched_resumes if r.uploaded_for_id == jd.id]
    pool_unmatched_resumes = [r for r in unmatched_resumes if r.uploaded_for_id != jd.id]
    ...
    return {
        ...
        'own_unmatched_resumes': own_unmatched_resumes,
        'pool_unmatched_resumes': pool_unmatched_resumes,
...
def update_job_status(request, job_description_id):
    ...
    cross-role recommendation target (see _build_shortlist_context's
    other_role_matches filter).
    """
    jd = get_object_or_404(JobDescription, pk=job_description_id)
    if request.method == 'POST':
        ...
        for f in files:
            ...
            if existing:
                if existing.redacted_text:
                    ...
                    continue
                # A prior upload of this same content was created but never
                # finished extraction (e.g. a transient Gemini failure) --
                # content_hash is set before extraction runs, so without
                # this check a re-upload would be silently skipped as a
                # "duplicate" forever, with no way to actually retry it.
                try:
                    pipeline.process_resume(existing)
                except Exception:
                    logger.exception('Retry extraction failed for Resume %s (%s)', existing.id, f.name)
                    messages.warning(request, f'{f.name}: retry failed again, check server logs.')
                else:
                    messages.success(request, f'{f.name}: extraction retried successfully.')
                continue

            resume = Resume.objects.create(file=f, raw_text=raw_text, content_hash=content_hash, uploaded_for=jd)
            try:
                pipeline.process_resume(resume)
            except Exception:
...
def run_matching(request, job_description_id):
    """Matches unscored resumes against this job. Defaults to only resumes
    originally uploaded for THIS job (Resume.uploaded_for) -- matching the
    entire shared resume pool against every job by default meant clicking
    "Run Matching" on a brand-new job could silently re-score dozens of
    unrelated candidates from earlier jobs (4+ LLM calls each), which was
    enough on its own to overload Gemini's free tier and blow through
    gunicorn's request timeout. Pass ?scope=pool to explicitly opt into
    checking the wider pool instead -- same "not hidden, just not default"
    principle as the interview-target planner.
    """
    jd = get_object_or_404(JobDescription, pk=job_description_id)
    if request.method == 'POST':
        matched_resume_ids = set(jd.matches.values_list('resume_id', flat=True))
        pool_scope = request.POST.get('scope') == 'pool'
        candidates = Resume.objects.all() if pool_scope else Resume.objects.filter(uploaded_for=jd)
        resumes = list(candidates.exclude(id__in=matched_resume_ids))
        if not resumes:
            messages.info(request, 'All eligible resumes are already matched against this job.')

...
def run_match(resume, job_description):
    ...
    scores = scoring.score_all_dimensions(
        ...
        eligibility_criteria=requirements.get("eligibility_criteria", []),
    )
    ...
    match, _ = Match.objects.update_or_create(
        ...
        defaults=dict(
            ...
            is_eligible=scores.eligibility.eligible,
            eligibility_details=[c.model_dump() for c in scores.eligibility.criteria],
            skill_evidence=[e.model_dump() for e in scores.skill_evidence],
        ),
    )
    return match

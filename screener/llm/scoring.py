...
skill-overlap anchor, blended into the final hybrid score. Also runs the
hard eligibility gate (min CGPA/percentage, graduation status, mandatory
experience cutoffs) as extracted into JobRequirements.eligibility_criteria.

Each dimension (skills/experience/education/eligibility) is grounded
independently: the retriever pulls only the resume excerpts relevant to
that dimension. The LLM call itself is consolidated into a single request
that scores all of them together (see score_all_dimensions) -- retrieval
is still per-dimension so results stay evidence-grounded, this only cuts
API round-trips, which matters a lot against a tight free-tier rate limit.
...
from .client import get_chat_model, with_llm_retry
...
COMBINED_SCORING_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        "another.\n\n"
        "Additionally, grade EVERY individual skill listed in Required "
        "skills (not just the overall skills score) into skill_evidence, "
        "using only the skills excerpts: Strong = clearly and directly "
        "demonstrated with specific supporting detail (built something "
        "with it, quantified impact, etc.); Partial = mentioned in a "
        "relevant context but not clearly demonstrated in depth; Weak = "
        "only a self-declared buzzword or bare skills-list mention with no "
        "supporting evidence in context -- self-declared buzzwords are "
        "weak evidence, not proof; None = not found anywhere in the "
        "excerpts. Include one skill_evidence entry per required skill, "
        "using its exact name, with a supporting quote where available.\n\n"
        "Additionally, check the candidate against each hard eligibility "
        "criterion listed below, using only the eligibility excerpts. Mark "
        "a criterion met=true only if the excerpts clearly satisfy it -- "
        "if the resume doesn't address what's needed to verify a "
        "criterion, mark it met=false with evidence 'not stated in "
        "resume' (fail closed, don't assume it's fine). eligible is true "
        "only if every criterion is met, or true if no criteria are "
        "listed.",
    ),
    (
        "human",
        ...
        "Retrieved excerpts:\n{education_context}\n\n"
        "== ELIGIBILITY ==\n"
        "Hard eligibility criteria: {eligibility_criteria}\n"
        "Retrieved excerpts:\n{eligibility_context}",
    ),
])
...
DIMENSION_QUERIES = {
    ...
    "eligibility": (
        "education percentage CGPA aggregate marks grades graduation "
        "degree completion status years of experience"
    ),
}
...
def score_all_dimensions(
    ...
    chain = with_llm_retry(COMBINED_SCORING_PROMPT | model)
    return chain.invoke({
        ...
        "eligibility_criteria": format_eligibility_criteria(eligibility_criteria),
        "eligibility_context": context_for("eligibility"),
    })
...
def format_eligibility_criteria(eligibility_criteria) -> str:
    if not eligibility_criteria:
        return "none specified"
    return "; ".join(eligibility_criteria)
